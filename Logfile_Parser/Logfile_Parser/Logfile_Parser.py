...
def ParseLogfileMFTRecord():
    ...
    mftRecord = drive.read(mftRecordSize)
    mftMirror = drive.read(mftRecordSize)
    logfile = drive.read(mftRecordSize)
    ...
    print("\n$Data")
    dataRun = str(binascii.hexlify(logfile[firstAttOffset+272:firstAttOffset+280]))

    lengthBytes = int(dataRun[3])
    offsetBytes = int(dataRun[2])
    length = dataRun[4:4+(lengthBytes*2)]
    offset = dataRun[4+(lengthBytes*2):4+(lengthBytes*2)+(offsetBytes*2)]
    print(length)
    print(offset)
...
def GetLogfileData():
    drive = open(r"\\.\C:","rb")
    drive.seek(512*8*15513867)

    logfileData = drive.read(512*8*64)

    return logfileData

if __name__ == "__main__":
#    ParseLogfileMFTRecord()
    logfile = GetLogfileData()
